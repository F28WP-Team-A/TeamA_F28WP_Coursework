...
var Shield = function (initPack) {
  ...
  self.draw = function () {
    var width = Images.shield.width * 2;
    var height = Images.shield.height * 2;
...
socket.on("init", function (data) {
  ...
  for (var i = 0; i < data.shield.length; i++) {
    new Shield(data.shield[i]);
  }
});
...
socket.on("update", function (data) {
  ...
  for (var i = 0; i < data.shield.length; i++) {
    var pack = data.shield[i];
    var s = Shield.list[data.bullet[i].id];
    if (s) {
      if (pack.x !== undefined) s.x = pack.x;
      if (pack.y !== undefined) s.y = pack.y;
    }
  }
});
...
socket.on("remove", function (data) {
  ...
  for (var i = 0; i < data.shield.length; i++) {
    delete Shield.list[data.shield[i]];
  }
});
...
setInterval(function () {
  ...
  map();
  playerScore();
  for (var i in Shield.list) Shield.list[i].draw();
